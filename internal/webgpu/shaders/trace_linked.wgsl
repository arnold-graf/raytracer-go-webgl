...
// modules-sha256: 227451f32b3af7b8afe59b285fd2674359dbd0fc301cd36026be2bdab4da1d44
...
fn ray_color(origin: vec3<f32>, dir0: vec3<f32>) -> vec3<f32> {
  ...
  loop {
    ...
    if (hit.kind == 0u) {
      ...
      let cone_cap = p.info.x == PRIM_CONE && abs(lhp.y - p.geo_a.w) < 0.02;
      if (cone_cap) {
...
fn hit_cone(p: Prim, ro: vec3<f32>, rd: vec3<f32>) -> f32 {
  ...
  // Base disk cap — always considered so the nearer of side vs cap wins
  // (entering through the flat base must not pick a farther side exit).
  if (abs(dy) > 1e-6) {
