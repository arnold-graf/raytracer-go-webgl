...
// modules-sha256: 0e3bce364da717893f16d938c4d6fb8401a17463333e56b09fde739b1eda5aa8
    ...
    if (mat == MAT_GLASS && reflective) {
      ...
      if (thin) {
        ...
        if (!tir && w_refl < 0.98 && w_trans > SEG_MIN_CONTRIB && sp < MAX_SEGS) {
          ...
          let enter_hp = hp - n * SURFACE_EPSILON;
          ...
          if (k_out >= 0.0) {
            ...
            stack[sp] = RaySeg(exit_hp + exit_n * SURFACE_EPSILON, rd_out, tw * alb * (1.0 - reflectance), depth + 1u, 0u);
            sp = sp + 1u;
          }
        }
      } else {
        ...
        if (!tir && w_refl < 0.98 && w_trans > SEG_MIN_CONTRIB && sp < MAX_SEGS) {
          ...
          stack[sp] = RaySeg(hp - n * SURFACE_EPSILON, rr, tw * alb * (1.0 - reflectance), depth + 1u, 0u);
          sp = sp + 1u;
        }
      }
...
fn shade_ghost(hp: vec3<f32>, alb: vec3<f32>, n: vec3<f32>, ep: vec3<f32>) -> vec3<f32> {
  ...
  var lit = alb * scene_ambient(n);
  for (var i = 0u; i < params.light_count; i = i + 1u) {
    lit = add_point_light(lit, hp, alb, n, ep, lights[i], tw_skip);
  }
  return lit;
}
