  ...
  loop {
    ...
    if (hit.kind == 0u) {
      ...
      if (p.info.x == PRIM_BOX) {
        alb = texture_eval_box(hit.idx, tex_p, tex_n, p.geo_a.xyz, p.geo_b.xyz, p.info.z, p.albedo.xyz);
      } else {
...
@compute @workgroup_size(8, 8, 1) fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let dir = normalize(params.fwd.xyz + params.right.xyz * (u * params.aspect * params.fov_scale) + params.up.xyz * (v * params.fov_scale));
