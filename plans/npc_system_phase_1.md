...
### Planned optimizations (from [`plans/dynamic-objects.md`](plans/dynamic-objects.md))
1. **Dirty transform upload** — track changed primitive index ranges; upload only `Xform` GPU rows for `DynamicBodies`
2. **BVH refit** — refit acceleration structure for moved boxes/cylinders instead of full rebuild
3. **Update cadence** — optional lower rate for distant agents (every 2–3 frames)
4. **Instancing** — same rig path → shared topology, per-instance transform buffer (larger change)

### Milestones
| Step | Target |
|------|--------|
| 5a | Dirty flag per `DynamicBody`; skip `Touch()` full rebuild when only transforms change |
| 5b | Partial GPU buffer write for owned ranges |
| 5c | 10 agents walking, < 2 ms CPU pose + upload on M-series |
