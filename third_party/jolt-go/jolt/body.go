...
// GetRotation returns the body's orientation quaternion.
func (bi *BodyInterface) GetRotation(bodyID *BodyID) Quat {
	var qx, qy, qz, qw C.float
	C.JoltGetBodyRotation(bi.handle, bodyID.handle, &qx, &qy, &qz, &qw)
	return Quat{X: float32(qx), Y: float32(qy), Z: float32(qz), W: float32(qw)}
}

// SetPositionAndRotation sets pose; activate wakes the body when true.
func (bi *BodyInterface) SetPositionAndRotation(bodyID *BodyID, position Vec3, rotation Quat, activate bool) {
	act := C.int(0)
	if activate {
		act = 1
	}
	C.JoltSetBodyPositionAndRotation(
		bi.handle,
		bodyID.handle,
		C.float(position.X),
		C.float(position.Y),
...
// CreateBodyEx creates a body with full material and mass settings.
// massKg is total mass in kilograms; 0 uses shape-derived mass.
func (bi *BodyInterface) CreateBodyEx(shape *Shape, position Vec3, rotation Quat, motionType MotionType, isSensor bool, massKg, friction, restitution float32, allowSleeping bool) *BodyID {
	sensor := C.int(0)
	if isSensor {
		sensor = 1
	}
...
// SetSensor toggles sensor mode (overlap without contact response).
func (bi *BodyInterface) SetSensor(bodyID *BodyID, isSensor bool) {
	sensor := C.int(0)
	if isSensor {
		sensor = 1
	}
	C.JoltSetBodySensor(bi.handle, bodyID.handle, sensor)
}

// IsActive reports whether the body is awake in the simulation.
...
func (bi *BodyInterface) ApplyImpulse(bodyID *BodyID, point, impulse Vec3) {
	C.JoltApplyImpulse(
		bi.handle,
		bodyID.handle,
		C.float(point.X),
...
// RemoveAndDestroy removes the body from the world and frees its ID.
